...
df = (
    ...
    .with_columns(
        pl.col("equation").list.len().sub(1).alias("n_combinations"),
    )
)
    ...
    pl.DataFrame(
        {
            "combinations": [
                ...
                for p in product(POSSIBLE_OPERATORS.keys(), repeat=r)
            ]
        }
    )
    .with_columns(
        pl.col("combinations").list.len().alias("n_combinations"),
    )
    .with_columns(
        pl.cum_count("n_combinations")
        .over("n_combinations")
        .sub(1)
...
operations_pydict = operations.select(
    "index_equation",
    "index_combination",
    "equation",
    pl.col("combinations").alias("operators"),
...
print(
    ...
    .agg(
        pl.col("value").first(),
        (pl.col("value") == pl.col("results")).any().alias("is_valid"),
    )
    .filter(pl.col("is_valid"))["value"]
    .sum()
)
