...
df_blocks = (
    ...
    .with_columns(
        ...
        .then(
            pl.col('index').repeat_by(pl.col('disk_map'))
        )
        .otherwise(
            pl.lit('.').repeat_by(pl.col('disk_map'))
        )
        .alias('n'),
    )
    .explode('n')
...
while True:
    ...
    index_to_substitute = len_blocks - 1 - (
        list(reversed(flag_blocks))
        .index(True)
    )
...
print(
    ...
    .with_row_index()
    .select(
        (pl.col('blocks') * pl.col('index'))
        .alias('filesystem_checksum')
    )
    ['filesystem_checksum']
    .sum()
)
