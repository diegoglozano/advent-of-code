...
print(
    ...
    .with_columns(
        pl
        .col('row')
        .sub(pl.col('row_right'))
        .alias('row_distance'),
        pl
        .col('column')
        .sub(pl.col('column_right'))
        .alias('column_distance'),
    )
    .with_columns(
        (pl.col('row') + pl.col('row_distance'))
        .alias('antinode_1_row'),
        (pl.col('column') + pl.col('column_distance'))
        .alias('antinode_1_column'),
        (pl.col('row_right') - pl.col('row_distance'))
        .alias('antinode_2_row'),
        (pl.col('column_right') - pl.col('column_distance'))
        .alias('antinode_2_column'),
    )
    .pipe(
        lambda x: (
            pl
...
temp = (
    ...
    .with_columns(
        (pl.col('row') + pl.col('row_distance').mul(i))
        .alias(f'antinode_1{i}_row')
        for i in range(1, n_rows)
    )
    .with_columns(
        (pl.col('column') + pl.col('column_distance').mul(i))
        .alias(f'antinode_1{i}_column')
        for i in range(1, n_cols)
    )
    .with_columns(
        (pl.col('row_right') - pl.col('row_distance').mul(i))
        .alias(f'antinode_2{i}_row')
        for i in range(1, n_rows)
    )
    .with_columns(
        (pl.col('column_right') - pl.col('column_distance').mul(i))
        .alias(f'antinode_2{i}_column')
        for i in range(1, n_cols)
    )
    .pipe(
        lambda x: (
