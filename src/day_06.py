...
x = int((input_array == current_direction).argmax(axis=0).max())
y = int((input_array == current_direction).argmax(axis=1).max())
original_x = x
original_y = y
positions = set()
while True:
...
print(len(set(position[0:2] for position in positions)))
